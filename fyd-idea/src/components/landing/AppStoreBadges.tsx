...
export function AppStoreBadges({ variant = "dark", className = "" }: Props) {
  ...
  const base =
    "group inline-flex items-center gap-3 rounded-2xl px-5 py-3 min-h-14 transition-all hover:scale-[1.02] active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand";
  const skin = isDark
    ? "bg-white text-zinc-950 hover:bg-zinc-100"
    : "bg-zinc-950 text-white border border-white/15 hover:bg-zinc-900";

  return (
    <div className={`flex flex-wrap gap-3 ${className}`}>
      <a
        href="#"
        aria-label="Download FirstDate on the App Store"
        className={`${base} ${skin}`}
      >
        <svg className="size-7 shrink-0" viewBox="0 0 384 512" fill="currentColor" aria-hidden="true">
      ...
      <a
        href="#"
        aria-label="Get FirstDate on Google Play"
        className={`${base} ${skin}`}
      >
        <svg className="size-7 shrink-0" viewBox="0 0 512 512" aria-hidden="true">
          ...
          <path fill="#4285F4" d="M385.4 174.2 467 222c17.7 10.2 17.7 35.8 0 46L385.4 315.8l-66.1-66.1 66.1-75.5z"/>
          <path fill="#EA4335" d="M104.6 13c-4.3 2.8-7.1 7.7-7.1 14.2v457.6c0 6.5 2.8 11.5 7.1 14.2l218.7-243-218.7-243z"/>
        </svg>
        <span className="flex flex-col items-start leading-tight">
